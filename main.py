    ...
    for path, folders, files in os.walk(directory):
        for filename in files:
            try:
                ...
                extension = None
            if filename == "AndroidManifest.xml" or extension in ["java", "smali"]:
...
if __name__ == "__main__":
    ...
    # Read the results from flagged_files.json
    with open("flagged_files.json", "r") as outfile:
        data = json.load(outfile)

    # Generate respective output files based on arguments
    if args.permissions:
    ...
    # Handle very verbose mode separately
    if args.very_verbose:
        # Permissions
        table = PrettyTable(["File_name", "Permissions"])
        for key in data:
        ...
        for key in data.keys():
            if "AndroidManifest.xml" in key:
                ...
                with open('Android_manifest_examined.txt', 'w') as f:
                    f.write(table_data)
                print("Saved output as Android_manifest_examined.txt")
                table.clear()
                data.pop(key)
                break
        ...
        table = PrettyTable(["File_name", "url", "logging", "extra"])
        ...
        with open('others.txt', 'w') as f:
            f.write(table.get_string())
        print("Saved output as others.txt")
        table.clear()